...
Many attempts already tried to assess robustness or stability properties of graph metrics.
...
I review background material on graph theory (based mostly on~\cite{Estrada2017}) and fix terms to later avoid ambiguity.
Later, I introduce graph metrics, ranking, and metric robustness.
...
Graphs I use in this project are all \textsl{simple graphs}, i.e., edges are undirected (or bi-directional) and without loops.
...
Metrics are essentially functions of graphs, assigning a real value to each node.
...
\begin{minipage}{\linewidth}
    ...
    One of the simplest metrics is the \textsl{degree} nodes, often considered as a centrality measure.
    \vspace*{-1mm}
    \begin{flalign}
...
\begin{wrapfigure}[10]{R}{0.23\linewidth}
...
This definition means that ranking induced by a metric is \textsl{one of possible} orderings of nodes such that nodes with high metric values correspond to high ranks.
...
The Paper calculates robustness by experiments specifically on a few protein interaction networks (which presumably follow a similar structure), and then even dares to calculate the averaged robustness.
...
The Paper and my dissertation both use for simplicity only the overall score, disregarding biological details.
...
Often, there is often only one instance of a graph to work with, so to simulate having multiple \footnote{Here, ``similar'' means that they convey the same real-world structure.} graphs, we generate multiple samples with small perturbations.
...
\begin{minipage}{\linewidth}
    ...
    Still, the threshold is a hyper-parameter of the graph-obtaining process and robust metrics should respond similarly to all reasonable values of the chosen threshold.
\end{minipage}
...
\begin{align}
    &\mathlarger{^T \mathlarger{\xi}}_{\mu}^{\nu}(G) \generates{n} (V, E_i)~\text{for}~i\in \integersto{n}, \\[2pt]
...
Further, the 3 robustness measures I used are based on comparing sets of the highest-ranked nodes, so deleting edges and preserving the set of nodes is more suitable than deleting nodes.\footnote{Moreover, deleting nodes could introduce more drastic perturbations as randomly deleting a high-degree node is a more extreme change than deleting a low-degree node, thus causing additional bias in robustness.}
...
\begin{align}
    &\mathlarger{^{ER} \mathlarger{\xi}}_\delta(G) \generates{n} G_1, G_2, \dots, G_n, \label{eq:edge_removing_generator}\\
    \text{where}~& G_1 = G\,, \nonumber \\
...
Unlike linear thresholding, edge removing applies to unscored networks.
...
Further, define \textsl{overall ranking} on a sequence of perturbed graphs that is calculated by first averaging node ranks across all induced rankings and then ranking the averaged values.
...
Taking $\alpha$-relaxed $k$-similarity\footnote{The Paper uses slightly different value of $k$ for each dataset, so in order to generalise this process I set $k$ to be a constant value calculated as the average of the values used in The Paper: $k \approx 0.023478$.} values between ranks of perturbed graphs and an overall ranking (induced by a metric), rank identifiability is defined as their minimum.
...
Finally, \textsl{rank instability} measures how much the rank of each node changes.
