...
Secondly, the rank identifiability measure attempts to quantify, how well ranks imposed by a metric on perturbed graphs resemble the overall ranking.
...
\begin{definition}[Rank identifiability]
    ...
    \begin{flalign*}
        &\operatorname{rank\ identifiability} \args \eqdef \min\limits_{i \in \integersto{n}} M_k^\alpha(A^{M(G_{\mathlarger{i}})}, A^* \mid k) \\[10pt]
        ...
        & \alpha = 1.5, \nonumber \\
        & M_k^\alpha(\boldsymbol{\cdot}, \boldsymbol{\cdot} \mid k)\ \ \text{is $\alpha$-relaxed $k$-similarity of two rankings, by~\autoref{def:k_similarity}} \nonumber
    \end{flalign*}
\end{definition}
\end{savenotes}
