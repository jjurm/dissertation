...
One of the most common centrality measures, betweenness centrality, is calculated for each node $v$ as the number of all shortest paths passing through that node, for all pairs of nodes.
...
\begin{definition}[Graph matching]
    ...
    \begin{equation}
        \forall v_1 \in V_1, \forall v_2 \in V_2.\quad v_1 \leftrightsquigarrow v_2\quad\text{iff}\quad \begin{array}{l}v_1, v_2\ \text{correspond to the same }\\\text{node in the source network}\end{array}
    \end{equation}
\end{definition}
...
Given a metric $M$, a source graph $G_0$, a graph generator $\xi$, and the number of perturbed graphs $n$, let \textbf{metric evaluation} refer to the application of a metric to a sequence of generated graphs:
\begin{align}
...
\begin{definition}[$\boldsymbol{k}$-similarity]
...
\begin{definition}[$\boldsymbol{\alpha}$-relaxed $\boldsymbol{k}$-similarity]
...
The following three rank robustness measures are defined given a metric $M$, a source graph $G_0$, a graph generator $\xi$, and the number of perturbed graphs $n$
...
\begin{definition}[Rank instability]
    ...
    \begin{flalign}
        &\operatorname{rank\ instability} \args \eqdef \frac{1}{\left\| U \right\|}\, \mathlarger{\sum}_{v \in U} \frac{range(v)}{n} & \\[10pt]
...
In the definition, $U$ is the set of the $1\%$ highest ranked nodes according to the overall ranking $A^*\!\left(M \,|\, G, \xi, n \right)$.
