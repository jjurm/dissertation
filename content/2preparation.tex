...
\autoref{fig:histogram_edges} shows the distribution of confidence scores in 3 protein interaction networks that are investigated in my work (see \autoref{sec:datasets}).
...
When we apply a confidence threshold to a scored network, discarding the scores, the resulting subgraph with a filtered set of edges is also an unscored network.

There are numerous sources providing interesting unscored network datasets, freely available online.

\paragraph*{Stanford Large Network Dataset Collection~\cite{Large2016}} Provides open datasets obtained from real-world data such as social networks, citation networks, web graphs, internet networks, road networks and many more.
...
I picked a number of datasets to work with (see the~\nameref{ch:evaluation} chapter) to demonstrate the concept.
However, the \graffs tool is universal and not bound to any set of datasets.
I chose the following unscored datasets, considering mainly their size (number of nodes and density; due to associated computational cost), and supposed structure:
\begin{description}[itemsep=\zerospace]
    \item[\texttt{airports}]\ Airport–airport flights in the US in 2010~\cite{konect:2016:opsahl-usairport,konect:opsahl11}, from KONECT
    \item[\texttt{citation}]\ High-energy physics theory citation network~\cite{LeskovecGraphsTimeDensification2005,GehrkeOverview2003KDD2003}, from SNAP Datasets~\cite{Large2016}
    \item[\texttt{collab}]\ Collaboration network of Arxiv General Relativity category~\cite{LeskovecGraphEvolutionDensification2007}, SNAP Datasets
    \item[\texttt{facebook}]\ Social circles (friends) from Facebook~\cite{NIPS2012_4532}, from SNAP Datasets
    \item[\texttt{internet}]\ Internet topology network of autonomous systems~\cite{konect:zhang05,konect:2016:topology}, from KONECT
\end{description}

There are a number of huge online repositories providing access to thousands networks (outside of the scope of this work), such as the Network Repository~\cite{RossiNetworkDataRepository2015}.
...
\todo{generate array of ~8 linearly thresholded graphs to demonstrate this}
...
\todo{mention \textsl{initial threshold} parameter}
...
\todo{extend these to unscored networks}
