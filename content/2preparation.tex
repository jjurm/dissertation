...
I explain in detail why measuring robustness of graph metrics is an important and novel field, describing the work done in The Paper, then I introduce definitions of graph metrics and robustness measures used in the implementation.


\section{Graphs and metrics}

Ever-increasing data sources and their applications force researchers to use tools such as graph theory to model real-world problems, such as in information technology, engineering, biology, chemistry, social systems, geography, linguistics and many more~\cite{FouldsGraphTheoryApplications2012}.
...
Further, graph \textsl{metrics} (such as betweenness centrality, closeness centrality, eccentricity) quantify properties of individual nodes.\footnotemark{}
\footnote{For example a person in a social network represented by a node with high closeness centrality is considered to be \textsl{closer} to others in terms of number of hops needed, in other words can spread information in the network faster.}
...
This dissertation project builds on top of the ideas from The Paper by Bozhilova et al.~\cite{Bozhilova2019}, one of the most recent researches on metric robustness.
The study, with main concepts explained below, demonstrates instability of some graph metrics on thresholded protein networks and proposes ways to quantify robustness of graph metrics in such networks.
...
In this section I review background material on graph theory and fix terms to later avoid ambiguity.
...
Those are omitted here for brevity, but included in \cref{ch:graph_theory} for reference.
...
In this section I describe some of the metrics that are most significant for analysing protein interaction networks and other graphs.
...
The metrics introduced below are those implemented in the \graffs tool.
...
Note: the~\nameref{def:rank_continuity} robustness measure compares pairs of consecutively generated graphs (e.g. graphs thresholded at consecutive confidence threshold), that is why we define graph generator to output a sequence of graphs instead of a set.
...
One study in a similar context~\cite{BorgattiRobustnessCentralityMeasures2006} proposes four kinds of random error in networks:
\begin{itemize}[topsep=5pt]
    \item deleting edges,
    \item deleting nodes,
    \item adding edges,
...
Thus, random node deletion could have a significant bias in these robustness measures, whereas edge deletion does not (only indirectly, by affecting metric values).

For the reasons above, I decided to perturb graphs by randomly deleting edges.

\parspace

Define an \textbf{edge removing} graph generator, producing a sequence of graph, with a proportion of edges to delete $\delta$ (deletion rate) at each step:
\begin{align}
    &\mathlarger{^{ER} \mathlarger{\xi}}_\delta(G) \generates{n} G_1, G_2, \dots, G_n, \label{eq:edge_removing_generator}\\[6pt]
    \text{where}~& G_1 = G\,, \nonumber \\
    & \forall i \in \left\{2, \dots, n\right\}. \begin{array}[t]{l} \forall e \in E.\ \chi_e\sim\mathcal{U}(0,1)\,, \\ G_i = \left( V_{i-1}, \Set{ e \in E_{i-1} }{ \chi_e > \delta }  \right) \end{array} \nonumber \\
\end{align}
...
However, in practice, it is useful to specify an \textsl{initial threshold} so that this generator can work on scored networks too, by turning them into unscored first.
...
Consequently, following rank robustness measures are defined given a metric $M$, a base graph $G$, a graph generator $\xi$, and the number of perturbed graphs to generate $n$.

Speaking of scored networks, \textsl{rank continuity} of a metric is the similarity between rankings induced at consecutive thresholds.
...
This measure utilises the fact that graphs are generated in a sequence, with the number of edges monotonically decreasing.
...
Secondly, the \textsl{rank identifiability} measure attempts to quantify, how well ranks imposed by a metric on perturbed graphs resemble the overall ranking.
...
\begin{definition}[Rank identifiability]
    ...
    \vspace{-0.4cm}
    \begin{flalign*}
        &\operatorname{rank\ identifiability} \args \eqdef \min\limits_{i \in \integersto{n}} \operatorname{Sim}_k^\alpha(A^{M(G_{\mathlarger{i}})}, A^* ) \\[10pt]
...
Apart of the robustness measures above, there are other approaches one could investigate, outside of the scope of this work.
